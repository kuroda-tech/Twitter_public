...
## 概要
このプロジェクトは、Spring Boot と Java を使用して作成した "簡易 Twitter クローンアプリ" です。  
ユーザー登録・ログイン・投稿・一覧表示などの基本的な機能を備えており、Webアプリ開発の基礎からデータベース連携まで学べます。
...
- Spring Boot + Thymeleaf で MVC パターンを意識した設計  
- Spring Security によるユーザー認証の実装  
- JPA を用いたデータベース操作の理解  
- 投稿一覧やエラーメッセージ表示など、画面設計の工夫  
- Maven プロジェクト管理や GitHub への公開手順を習得
